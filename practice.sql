...
SELECT *
FROM countries
WHERE (code = 'DZA' OR code = 'ALB') AND
indep_year < 1990;
...
-- 「Arubaの人口は103000人です」
SELECT CONCAT(name, 'の人口は', population, '人です') AS country_population
FROM countries;
...
SELECT region, AVG(IFNULL(life_expectancy, 0)) AS 平均寿命, AVG(population) AS 平均人口
FROM countries
GROUP BY region
ORDER BY region ASC;
...
SELECT MIN(surface_area) AS 最小表面積
FROM countries
WHERE continent = 'Asia';

-- 問24
-- アジア大陸の表面積の合計を表示してください。
SELECT SUM(surface_area) AS 合計表面積
FROM countries
WHERE continent = 'Asia';

-- 問25
-- 全ての国と言語を表示してください。一つの国に複数言語があると思いますので同じ国名を言語数だけ出力してください。
SELECT countries.name AS name, countrylanguages.language
FROM countries
